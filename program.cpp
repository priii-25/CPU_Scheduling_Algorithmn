...
void priorityScheduling(vector<Process>& processes) {
    ...
    while (completed < processes.size()) {
        ...
        if (!pq.empty()) {
            ...
            processes[p.id - 1] = p;  // Update original process
            printProcess(p);
            completed++;
        } else {
            currentTime++;
...
void hrrn(vector<Process>& processes) {
    ...
    auto compareResponseRatio = [](Process a, Process b) {
        double responseRatioA = (a.waitingTime + a.burstTime) / (double) a.burstTime;
        double responseRatioB = (b.waitingTime + b.burstTime) / (double) b.burstTime;
